...
document.getElementById('settings-btn')?.addEventListener('click', async () => {
    ...
    // Load configs from storage to ensure we have the latest
    const storage = await storageGet(['blockchain-config', 'ipfs-config']);
    
    // Update blockchain status
    updateBlockchainStatus();
    
    // Populate blockchain config fields
    if (storage['blockchain-config']) {
        const bc = storage['blockchain-config'];
        if (bc.contractAddress) {
            document.getElementById('contract-address-input').value = bc.contractAddress;
        }
        if (bc.privateKey) {
    ...
    // Populate IPFS config fields
    if (storage['ipfs-config']) {
        const ic = storage['ipfs-config'];
        if (ic.pinataJWT) {
            document.getElementById('web3-storage-token-input').value = ic.pinataJWT;
        }
        document.getElementById('ipfs-toggle').checked = ic.enabled || false;
    }
    
    // Update IPFS info display
    if (typeof getLastBackupInfo === 'function') {
        getLastBackupInfo().then(info => {
            if (info) {
                updateIPFSInfo(info.cid, info.timestamp);
            }
        });
    }
});
...
document.getElementById('save-password-btn')?.addEventListener('click', async () => {
    ...
    const operation = currentPasswordId ? 'password_updated' : 'password_added';
    await saveVault(operation);
    showScreen('dashboard-screen');
    renderPasswordList();
...
document.getElementById('delete-password-btn')?.addEventListener('click', async () => {
    if (confirm('Are you sure you want to delete this password?')) {
        ...
        await saveVault('password_deleted');
        showScreen('dashboard-screen');
        renderPasswordList();
    }
});
...
document.getElementById('auto-save-save-btn')?.addEventListener('click', async () => {
    ...
    await saveVault('password_added');
        ...
        try {
            ...
            if (logs.length === 0) {
                ...
                logsList.innerHTML = logs.map(log => {
                    const event = formatBlockchainEvent(log.operation);
                    const ipfsInfo = log.ipfsCID ? `
                        <div style="font-size: 0.7rem; color: var(--accent-primary); margin-top: 0.5rem;">
                            <span style="margin-right: 0.5rem;">☁️ IPFS:</span>
                            <span style="font-family: monospace;">${log.ipfsCID.substring(0, 30)}...</span>
                        </div>
                    ` : '';
                    return `
                    <div style="background: rgba(255,255,255,0.05); padding: 1rem; border-radius: 8px; margin-bottom: 0.75rem; border-left: 3px solid ${event.color};">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                            <span style="font-weight: 600; color: ${event.color}; font-size: 1rem;">
                                <span style="font-size: 1.25rem; margin-right: 0.5rem;">${event.icon}</span>
                                ${event.label}
                            </span>
                            <span style="font-size: 0.75rem; color: var(--text-muted);">${log.timestamp.toLocaleString()}</span>
                        </div>
                        <div style="font-size: 0.7rem; font-family: monospace; word-break: break-all; color: var(--text-muted); opacity: 0.7;">
                            Hash: ${log.hash}
                        </div>
                        ${ipfsInfo}
                    </div>
...
// Toggle IPFS configuration section
document.getElementById('toggle-ipfs-config-btn')?.addEventListener('click', () => {
    const configSection = document.getElementById('ipfs-config-section');
    if (configSection.style.display === 'none') {
        configSection.style.display = 'block';
    } else {
        configSection.style.display = 'none';
...
document.getElementById('save-ipfs-config-btn')?.addEventListener('click', async () => {
    const token = document.getElementById('web3-storage-token-input').value.trim();
    ...
    if (typeof enableIPFSBackup === 'function') {
        const result = await enableIPFSBackup(token);
        if (result.success) {
            showNotification('IPFS configuration saved!');
            document.getElementById('ipfs-config-section').style.display = 'none';
            updateIPFSStatus();
        } else {
            alert('Failed to save IPFS configuration');
        }
    } else {
        alert('IPFS module not loaded');
    }
});

// Test IPFS connection
document.getElementById('test-ipfs-btn')?.addEventListener('click', async () => {
    const token = document.getElementById('web3-storage-token-input').value.trim();
    ...
    // Temporarily enable with this token to test
    if (typeof enableIPFSBackup === 'function' && typeof testIPFSConnection === 'function') {
        await enableIPFSBackup(token);
...
// Backup to IPFS now
document.getElementById('backup-to-ipfs-btn')?.addEventListener('click', async () => {
    if (!vault.ipfsEnabled) {
        alert('Please enable IPFS backup in settings first');
        return;
    }
    ...
    try {
        // Trigger a save which will upload to IPFS
        await saveVault('manual_backup');
        showNotification('✅ Vault backed up to IPFS successfully!');
    } catch (error) {
...
// Restore from IPFS
document.getElementById('restore-from-ipfs-btn')?.addEventListener('click', async () => {
    const cidInput = prompt('Enter IPFS CID to restore from:\n\n(Or leave empty to use latest from blockchain)');
    
    if (cidInput === null) return; // User cancelled
    
    try {
        let cid = cidInput.trim();
        
        // If no CID provided, try to get latest from blockchain
        if (!cid && typeof getLatestAuditLog === 'function') {
            console.log('Fetching latest CID from blockchain...');
            const latestLog = await getLatestAuditLog();
            if (latestLog && latestLog.ipfsCID) {
                cid = latestLog.ipfsCID;
                console.log('Found CID from blockchain:', cid);
            } else {
                alert('No IPFS CID found in blockchain. Please enter CID manually.');
                return;
            }
        }
        ...
        // Download from IPFS
        console.log('📥 Restoring vault from IPFS...');
        showNotification('Downloading from IPFS...');
        
        if (typeof downloadFromIPFS === 'function') {
            ...
            // Ask for mnemonic to decrypt
            const mnemonic = prompt('Enter your 12-word recovery phrase to decrypt:');
            if (!mnemonic) {
                alert('Recovery phrase required to decrypt vault');
                return;
            }
            ...
            if (!salt) {
                throw new Error('Salt not found in backup');
            }
            
            const recoveryKey = await deriveMasterKey(mnemonic, salt);
            
            // Decrypt vault
            const decryptedJSON = await decryptData(encryptedVault, recoveryKey);
            const recoveredVault = JSON.parse(decryptedJSON);
            
            // Verify hash against blockchain
            const computedHash = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(JSON.stringify(encryptedVault)));
            const latestLog = await getLatestAuditLog();
            
            if (latestLog && latestLog.hash !== computedHash) {
                if (!confirm('⚠️ Warning: Vault hash does not match blockchain record. Continue anyway?')) {
                    return;
                }
            }
            
            // Load recovered vault
            Object.assign(vault, recoveredVault);
            vault.mnemonic = mnemonic;
            vault.salt = salt;
            
            // Save locally
            await saveVault('vault_recovered');
            
            // Update UI
            renderPasswordList();
            showNotification('✅ Vault restored from IPFS successfully!');
            console.log('✅ Vault recovered from IPFS');
...
// Update IPFS status in UI
function updateIPFSStatus() {
    const statusSpan = document.getElementById('ipfs-status');
    if (typeof isIPFSEnabled === 'function') {
        if (isIPFSEnabled()) {
            statusSpan.textContent = '✅ Configured & Active';
            statusSpan.style.color = 'var(--accent-success)';
        } else {
...
// Update IPFS info display
async function updateIPFSInfo(cid, timestamp) {
    document.getElementById('last-cid').textContent = cid ? cid.substring(0, 20) + '...' : 'N/A';
    document.getElementById('last-backup-time').textContent = timestamp || 'N/A';
}

// Format blockchain event with icon and user-friendly name
function formatBlockchainEvent(operation) {
    const eventMap = {
        'vault_created': { icon: '🆕', label: 'Vault Created', color: '#4CAF50' },
        'password_added': { icon: '🔑', label: 'Password Added', color: '#2196F3' },
        'password_updated': { icon: '✏️', label: 'Password Updated', color: '#FF9800' },
        'password_deleted': { icon: '🗑️', label: 'Password Deleted', color: '#F44336' },
        'pin_changed': { icon: '🔐', label: 'PIN Changed', color: '#9C27B0' },
        'settings_updated': { icon: '⚙️', label: 'Settings Updated', color: '#607D8B' },
        'vault_updated': { icon: '💾', label: 'Vault Updated', color: '#00BCD4' },
        'vault_recovered': { icon: '🔄', label: 'Vault Recovered', color: '#FF5722' },
        'vault_imported': { icon: '📥', label: 'Vault Imported', color: '#3F51B5' },
        'save': { icon: '💾', label: 'Saved', color: '#00BCD4' }
    };
    return eventMap[operation] || { icon: '📝', label: operation, color: '#9E9E9E' };
}
    ...
    try {
        ...
        reader.onload = async (event) => {
            try {
                ...
                // Save merged vault
                await saveVault('vault_imported');
                renderPasswordList();
...
async function saveVault(operation = 'vault_updated') {
    try {
    ...
    console.log('Encrypted vault object (ready to store):', encryptedVault);
        ...
        // ☁️ IPFS BACKUP (if enabled)
        let ipfsCID = '';
        if (vault.ipfsEnabled && typeof uploadToIPFS === 'function') {
            try {
                console.log('📤 Uploading vault to IPFS...');
                const ipfsResult = await uploadToIPFS(storedVault);
                
                if (ipfsResult.success) {
                    ipfsCID = ipfsResult.cid;
                    console.log('✅ Vault uploaded to IPFS:', ipfsCID);
                    updateIPFSInfo(ipfsCID, new Date().toLocaleString());
                } else {
                    console.warn('⚠️ IPFS upload failed:', ipfsResult.error);
                }
            } catch (ipfsError) {
                console.error('IPFS backup error:', ipfsError);
                // Don't fail the save operation if IPFS backup fails
            }
        }
        
        // 🔗 BLOCKCHAIN AUDIT LOGGING (with IPFS CID if available)
        if (vault.auditEnabled && typeof logVaultWithIPFS === 'function') {
            try {
                const encryptedJSON = JSON.stringify(storedVault);
                const blockchainResult = await logVaultWithIPFS(encryptedJSON, ipfsCID, operation);
                
                if (blockchainResult.success) {
                    console.log('✅ Vault logged to blockchain:', blockchainResult.hash);
                    if (ipfsCID) {
                        console.log('📦 IPFS CID logged to blockchain:', ipfsCID);
                    }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        // Load blockchain and IPFS configurations FIRST
        const storage = await storageGet(['blockpass-vault', 'blockpass-unlock-token', 'pendingSave', 'blockchain-config', 'ipfs-config']);
