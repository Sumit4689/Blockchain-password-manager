...
const CONTRACT_ABI = [
    "event VaultLogged(bytes32 indexed vaultHash, address indexed user, uint256 timestamp, string operation)",
    ...
    "function getUserLog(address _user, uint256 _index) external view returns (tuple(bytes32 vaultHash, address user, uint256 timestamp, string operation) memory)",
    "function getUserLogs(address _user) external view returns (tuple(bytes32 vaultHash, address user, uint256 timestamp, string operation)[] memory)",
    "function verifyHash(address _user, bytes32 _hash) external view returns (bool)",
    "function getLatestLog(address _user) external view returns (tuple(bytes32 vaultHash, address user, uint256 timestamp, string operation) memory)",
...
async function logVaultToBlockchain(encryptedVaultJSON, operation = "save") {
    ...
    try {
        ...
        // Send transaction
        const tx = await contract.logVault(vaultHash, operation);
